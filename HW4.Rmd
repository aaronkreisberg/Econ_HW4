...
```{r}
#social cost of carbon function??? not working.
scc_function <- function(x){
  scc <- warming_scenarios$pulse_damages_per_ton/
    I((1 + x)^(warming_scenarios$X1))
  return(scc)
}
...
discount_rate <- seq(0.01, 0.1, 0.001)

#create dataframe
scc <- scc_function(discount_rate)
...
ramsey_point <- data.frame(ramsey_discount_rate, scc_function(ramsey_discount_rate))

ggplot() +
  geom_point(data = scc, aes(x = discount_rate, y = scc)) +
  geom_point(data = ramsey_point, aes(x = ramsey_discount_rate, y = scc_function.ramsey_discount_rate., color = "red", size = 1))#
```
